class CreatePayTables < ActiveRecord::Migration[8.1]
  def change
    primary_key_type, foreign_key_type = primary_and_foreign_key_types

    create_table :pay_customers, id: primary_key_type, default: -> { "uuid()" } do |t|
    ...
    create_table :pay_merchants, id: primary_key_type, default: -> { "uuid()" } do |t|
      t.belongs_to :owner, polymorphic: true, index: false, type: foreign_key_type
      t.string :processor, null: false
      t.string :processor_id
      t.boolean :default
    ...
    create_table :pay_payment_methods, id: primary_key_type, default: -> { "uuid()" } do |t|
    ...
    create_table :pay_subscriptions, id: primary_key_type, default: -> { "uuid()" } do |t|
      t.belongs_to :customer, foreign_key: { to_table: :pay_customers }, null: false, index: false, type: foreign_key_type
    ...
    create_table :pay_charges, id: primary_key_type, default: -> { "uuid()" } do |t|
      t.belongs_to :customer, foreign_key: { to_table: :pay_customers }, null: false, index: false, type: foreign_key_type
    ...
    create_table :pay_webhooks, id: primary_key_type, default: -> { "uuid()" } do |t|
      t.string :processor
      t.string :event_type
  ...
  def primary_and_foreign_key_types
    ...
    primary_key_type = setting || :string
    foreign_key_type = setting || :string
